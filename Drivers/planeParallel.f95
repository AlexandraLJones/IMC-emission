...
program computePlaneParallelRT
  ...
  ! $URL: http://i3rc-monte-carlo-model.googlecode.com/svn/trunk/Example-Drivers/planeParallel.f95 $
  !
...
! ---------------------------------------------------
  ...
  real, dimension(:, :, :), allocatable &
                     :: fluxUp, fluxDown, fluxAbsorbed, temps
  ...
  subroutine createDomain
    ...
    allocate(            extinction(nX, nY, nLayers), &
             singleScatteringAlbedo(nX, nY, nLayers), &
                 phaseFunctionIndex(nX, nY, nLayers), temps(nX, nY, nLayers))
    temps(:,:,:) = 0.0
    !
    ! Define the domain
    !
